      ...
      <div className={cn("grid gap-1.5",
        ...
        {amenities.map((a) => (
          <AmenityCard
            key={a.id}
            amenity={a}
            isPresent={presentSet.has(a.id as number)}
            isPending={!!pending[a.id as number]}
            onToggle={() => onToggle(a.id as number, presentSet.has(a.id as number))}
            compact={compact}
          />
        ))}
      </div>
...
export function AmenityToggleGrid({ amenities, categories = [], presentSet, pending, onToggle, compact = false }: AmenityToggleGridProps) {
  const presentCount = amenities.filter((a) => presentSet.has(a.id as number)).length;
  ...
  for (const a of amenities) {
    const catId = a.categoryId as number;
    if (catId != null && catMap.has(catId)) {
      if (!grouped.has(catId)) grouped.set(catId, []);
      grouped.get(catId)!.push(a);
    } else {
